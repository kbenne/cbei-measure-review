module OsLib_HVAC
  ...
  def OsLib_HVAC.sortZones(model, runner, options = {}, space_type_to_edits_hash = {})
 ...
 def OsLib_HVAC.removeEquipment(model, runner, options)
    airloops = model.getAirLoopHVACs
    plantLoops = model.getPlantLoops
    zones = model.getThermalZones   
  ...
  def OsLib_HVAC.createHotWaterPlant(model, runner, hot_water_setpoint_schedule, loop_type, parameters)
  ...
  def OsLib_HVAC.createCondenserLoop(model, runner, options, parameters)
  ...
  def OsLib_HVAC.createPrimaryAirLoops(model, runner, options, parameters)
    primary_airloops = []
    ...
    model.getBuildingStorys.sort.each do |building_story|
      ...
      unless thermalZonesToAdd.empty?
        ...
        if options["primaryHVAC"]["heat"] == "Electric"
          # electric coil
  ...
  def OsLib_HVAC.createVRFAirConditioners(model, runner, options, parameters)
		vrf_AirConditioners = []
    # create primary airloop for each story
    assignedThermalZones = []
    model.getBuildingStorys.sort.each do |building_story|
   ...
   def OsLib_HVAC.createPrimaryZoneEquipment(model, runner, options, parameters)
