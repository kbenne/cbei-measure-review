module OsLib_HVAC
  ...
  def OsLib_HVAC.createHotWaterPlant(model, runner, hot_water_setpoint_schedule, loop_type)
    ...
    hot_water_plant.setName("#{loop_type} Loop")
    ...
    model.getBuildingStorys.sort.each do |building_story|
      ...
      unless thermalZonesToAdd.empty?
        ...
        thermalZonesToAdd.each do |zone|
		  ...
		  if options["primaryHVAC"]["fan"] == "Variable"
			heating_coil_vav_hw = OpenStudio::Model::CoilHeatingWater.new(model, model.alwaysOnDiscreteSchedule())
			air_terminal = OpenStudio::Model::AirTerminalSingleDuctVAVReheat.new(model, model.alwaysOnDiscreteSchedule(),heating_coil_vav_hw)
			options["hot_water_plant"].addDemandBranchForComponent(heating_coil_vav_hw) 
          else
			heating_coil_vav_electric = OpenStudio::Model::CoilHeatingElectric.new(model, model.alwaysOnDiscreteSchedule())
			air_terminal = OpenStudio::Model::AirTerminalSingleDuctVAVReheat.new(model, model.alwaysOnDiscreteSchedule(),heating_coil_vav_electric)
          end
    ...
    model.getThermalZones.each do |zone|
      if options["zonesSecondary"].include? zone
        ...
        if options["secondaryHVAC"]["heat"] == "Water"
          ...
          heating_coil = OpenStudio::Model::CoilHeatingWater.new(model, model.alwaysOnDiscreteSchedule())
          air_loop_comps << heating_coil
		elsif options["secondaryHVAC"]["heat"] == "Electric"
          # electric coil
          heating_coil = OpenStudio::Model::CoilHeatingElectric.new(model, model.alwaysOnDiscreteSchedule())
          air_loop_comps << heating_coil
        else  
        ...
        if options["secondaryHVAC"]["fan"] == "Variable"
			heating_coil_vav = OpenStudio::Model::CoilHeatingWater.new(model, model.alwaysOnDiscreteSchedule())
			air_terminal = OpenStudio::Model::AirTerminalSingleDuctVAVReheat.new(model, model.alwaysOnDiscreteSchedule(),heating_coil_vav)
			airLoopHVAC.addBranchForHVACComponent(air_terminal)
			options["hot_water_plant"].addDemandBranchForComponent(air_terminal) 
        else
