...
#start the measure
class VAVRTU_2SDX < OpenStudio::Ruleset::ModelUserScript 

  #define the name that a user will see, this method may be deprecated as
  #the display name in PAT comes from the name field in measure.xml
  def name
    return "Replace HVAC System with VAV RTU with 2-Stage DX Cooling, Hot Water or Electric Heating/Reheat, and ERV"
  end
  ...
  def arguments(model)
	...
	heatingcoilChs << "GasBoiler"
	heatingcoilChs << "ElectricResistance"
	heatingcoilType = OpenStudio::Ruleset::OSArgument::makeChoiceArgument("heatingcoilType",heatingcoilChs,true)
	heatingcoilType.setDisplayName("Heating Type")
	heatingcoilType.setDefaultValue("GasBoiler")
	args<<heatingcoilType
  
    # option of schedule type
	...
	# option of demand control ventilation (DCV)
	dcvcheck = OpenStudio::Ruleset::OSArgument::makeBoolArgument("dcvcheck",true)
    dcvcheck.setDisplayName("Use Demand Controlled Ventilation (DCV)?")
    dcvcheck.setDefaultValue(true)
    args << dcvcheck
  ...
  def run(model, runner, user_arguments)
	...
	dcvcheck = runner.getBoolArgumentValue("dcvcheck",user_arguments)
	...
	secondarySpaceTypeTest = ["Office"] # added for office
	
    primarySpaceType = "Office"
	if heatingcoilType == "GasBoiler"
	   primaryHVAC = {"doas" => false, "fan" => "Variable", "heat" => "Water", "cool" => "None"}
	   secondaryHVAC = {"fan" => "Variable", "heat" => "Water", "cool" => "TwoDX"} 
	   zoneHVAC = "None"
 	else
	   primaryHVAC = {"doas" => false, "fan" => "None", "heat" => "Electric", "cool" => "TwoDX"}
	   secondaryHVAC = {"fan" => "None", "heat" => "None", "cool" => "None"} 
	   zoneHVAC = "None"
	end
    ...
    ### START SORT ZONES
    options = {"standardBuildingTypeTest" => standardBuildingTypeTest, 
...
VAVRTU_2SDX.new.registerWithApplication