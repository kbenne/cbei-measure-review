  ...
  def OsLib_HVAC.createPrimaryAirLoops(model, runner, options)
    ...
    model.getBuildingStorys.sort.each do |building_story|
        ...
        if space.thermalZone.is_initialized
          ...
          if options["zonesPrimary"].include? thermal_zone
            ...
            unless assignedThermalZones.include? thermal_zone
			  ...
			  unless controlZonesToAdd.include? thermal_zone
				spaceType = space.spaceType.get
				unless options["primaryHVAC"]["controlZoneSpaceType"].nil?
					if spaceType == options["primaryHVAC"]["controlZoneSpaceType"]
						controlZonesToAdd << thermal_zone
					end
				end
			  end
      ...
      unless thermalZonesToAdd.empty?
        ...
        airloop_primary.setName("Air Loop HVAC #{building_story.name}")
		# find the supply inlet node of the airloop
        airloop_supply_inlet = airloop_primary.supplyInletNode
		airloop_supply_outlet = airloop_primary.supplyOutletNode 
        ...
        if options["primaryHVAC"]["doas"]
          ...
          sizing_system.setTypeofLoadtoSizeOn("Sensible") #VAV or CAV
          sizing_system.setAllOutdoorAirinCooling(false) #VAV or CAV 
          sizing_system.setAllOutdoorAirinHeating(false) #VAV or CAV 
        end    
        ...
        if options["primaryHVAC"]["fan"] == "Variable"
        ...
        elsif options["primaryHVAC"]["fan"] == "Constant"
		  ...
		  #create fan power ratio curve
		  fanPowerCurve = OpenStudio::Model::CurveExponent.new(model)
		  fanPowerCurve.setCoefficient1Constant(0.07385)
		  fanPowerCurve.setCoefficient2Constant(0.9246)		  
		  fanPowerCurve.setCoefficient3Constant(2.737)
		  fanPowerCurve.setMinimumValueofx(0)
		  fanPowerCurve.setMaximumValueofx(1.0)
		  
		  #create fan efficiency ratio curve
		  fanEfficiencyCurve = OpenStudio::Model::CurveQuadratic.new(model)
		  fanEfficiencyCurve.setCoefficient1Constant(1.0)
		  fanEfficiencyCurve.setCoefficient2x(0)
		  fanEfficiencyCurve.setCoefficient3xPOW2(0)
		  fanEfficiencyCurve.setMinimumValueofx(0)
		  fanEfficiencyCurve.setMaximumValueofx(1.0)
		  
		  fan.setFanPowerRatioFunctionofSpeedRatioCurve(fanPowerCurve)
		  fan.setFanEfficiencyRatioFunctionofSpeedRatioCurve(fanEfficiencyCurve)
        end
        unless options["primaryHVAC"]["multistage"]
		  air_loop_comps << fan
		end
		
        # create heating coil
		if options["primaryHVAC"]["multistage"]
			if options["primaryHVAC"]["heat"]== "Gas"
			  #two stage heating coil 
			  heating_coil = OpenStudio::Model::CoilHeatingGasMultiStage.new(model)
			  #heating_coil.setName("Multi Stage Gas Htg Coil")
			  heating_coil_stage_1= OpenStudio::Model::CoilHeatingGasMultiStageStageData.new(model)
			  heating_coil_stage_2= OpenStudio::Model::CoilHeatingGasMultiStageStageData.new(model)
			  heating_coil.addStage(heating_coil_stage_1)
			  heating_coil.addStage(heating_coil_stage_2)
			elsif options["primaryHVAC"]["heat"]== "Electric"
			  #electric resistance heating coil
			  #heating_coil.setName ("Electric Htg Coil")
			  heating_coil = OpenStudio::Model::CoilHeatingElectric.new(model, model.alwaysOnDiscreteSchedule())
			elsif options["primaryHVAC"]["heat"]== "DX Heatpump"
			  heating_coil = OpenStudio::Model::CoilHeatingDXMultiSpeed.new(model)
			  heating_coil.setFuelType("Electricity")
			  #heating_coil.setName("Multi Stage DX Heatpump Htg Coil")
			  heating_coil_stage_1 = OpenStudio::Model::CoilHeatingDXMultiSpeedStageData.new(model)
			  heating_coil_stage_1.setGrossRatedHeatingCOP(options["primaryHVAC"]["heatcoilCOP"])
			  heating_coil_stage_2 = OpenStudio::Model::CoilHeatingDXMultiSpeedStageData.new(model)
			  heating_coil_stage_2.setGrossRatedHeatingCOP(options["primaryHVAC"]["heatcoilCOP"])
			  heating_coil.addStage(heating_coil_stage_1)
			  heating_coil.addStage(heating_coil_stage_2)
			end 
		else
			...
			elsif options["primaryHVAC"]["heat"] == "Gas"  
			  # gas coil
        ...
        # create cooling coil
		if options["primaryHVAC"]["multistage"]
			#two stage DX Cooling Coil for unitary heatpump
			cooling_coil = OpenStudio::Model::CoilCoolingDXMultiSpeed.new(model)
			cooling_coil.setFuelType("Electricity")
			#cool.setName("Multi Stage DX Clg Coil")
			cooling_coil_stage_1 = OpenStudio::Model::CoilCoolingDXMultiSpeedStageData.new(model)
			cooling_coil_stage_1.setGrossRatedCoolingCOP(options["primaryHVAC"]["coolcoilCOP"])
			cooling_coil_stage_2 = OpenStudio::Model::CoilCoolingDXMultiSpeedStageData.new(model)
			cooling_coil_stage_2.setGrossRatedCoolingCOP(options["primaryHVAC"]["coolcoilCOP"])
			cooling_coil.addStage(cooling_coil_stage_1)
			cooling_coil.addStage(cooling_coil_stage_2)
			...
			if options["primaryHVAC"]["cool"] == "Water"
			  ...
			  cooling_coil = OpenStudio::Model::CoilCoolingDXSingleSpeed.new(model,
																			   ...
																			   clgPlrCurve)
			  cooling_coil.setRatedCOP(OpenStudio::OptionalDouble.new(options["primaryHVAC"]["coolcoilCOP"]))
			  air_loop_comps << cooling_coil
			else
		...
		# create supplemental heating coil for unitary heat pump
		if options["primaryHVAC"]["multistage"]
			supp_heat = OpenStudio::Model::CoilHeatingElectric.new(model,model.alwaysOnDiscreteSchedule())
		end 
		
		# create unitary system for multistage system
		if options["primaryHVAC"]["multistage"]
			unitary = OpenStudio::Model::AirLoopHVACUnitaryHeatPumpAirToAirMultiSpeed.new(model,fan,heating_coil,cooling_coil,supp_heat)
			unitary.setSupplyAirFanPlacement ("BlowThrough")
			unitary.addToNode(airloop_supply_outlet)
			if controlZonesToAdd.empty?
				unitary.setControllingZoneorThermostatLocation(thermalZonesToAdd[0])
			else
				unitary.setControllingZoneorThermostatLocation(controlZonesToAdd[0])
			end
		end 
		...
		unless options["zoneHVAC"] == "DualDuct"
          ...
          if options["primaryHVAC"]["doas"]
            ...
            if options["primaryHVAC"]["economizer"]
        ...
        unless (options["primaryHVAC"]["doas"] or options["zoneHVAC"] == "DualDuct" or options["primaryHVAC"]["fan"] == "Constant") or (options["primaryHVAC"]["fan"] == "OnOff")
		  ...
		  if (options["primaryHVAC"]["fan"] == "Constant") or (options["primaryHVAC"]["fan"] == "OnOff")
			#CAV Air Loop
			...
			if controlZonesToAdd.empty?
				setpoint_manager.setControlZone(thermalZonesToAdd[0])
			else
				setpoint_manager.setControlZone(controlZonesToAdd[0])
			end
		  else
        ...
        unless options["zoneHVAC"] == "DualDuct" or options["primaryHVAC"]["erv"]== false 
          heat_exchanger.addToNode(system_OA.outboardOANode.get)
        end  
        # add setpoint manager to supply equipment outlet node
        setpoint_manager.addToNode(airloop_supply_outlet)
