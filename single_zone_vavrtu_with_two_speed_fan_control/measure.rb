...
#start the measure
class SingleZoneVAVRTUWithTwoSpeedFanControl < OpenStudio::Ruleset::ModelUserScript

  #define the name that a user will see, this method may be deprecated as
  #the display name in PAT comes from the name field in measure.xml
  def name
    return " Single Zone VAV RTU with Two Speed Fan Control"
  end
  ...
  def arguments(model)
    ...
    # make an optional argument for plenum space type
	...
	# make an optional argument for control space type
    controlZoneSpaceType = OpenStudio::Ruleset::OSArgument::makeChoiceArgument("controlZoneSpaceType", usedSpaceTypes_handle, usedSpaceTypes_displayName,false)
    controlZoneSpaceType.setDisplayName("This space type should be set as the control zone for HVAC unit.")
    args << controlZoneSpaceType
	
	# make an argument for DX cooling coil rating COP
    coolingcoilCOP = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("coolingcoilCOP",true)
    coolingcoilCOP.setDisplayName("Cooling Coil Rating Condition COP")
    coolingcoilCOP.setDefaultValue(3.0)
    args << coolingcoilCOP
	
	# make an argument for DX cooling coil rating COP
    heatingcoilCOP = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("heatingcoilCOP",true)
    heatingcoilCOP.setDisplayName("DX Heating Coil Rating Condition COP")
    heatingcoilCOP.setDefaultValue(3.0)
    args << heatingcoilCOP
	
	# make an argument for heating coil type
	heatingcoilChs = OpenStudio::StringVector.new
	heatingcoilChs << "DX Heatpump"
	#heatingcoilChs << "Electric"  #OpenStudio currently has some problems with putting Electric heating coil in multispeed air to air heat pump as heating coil 
	heatingcoilChs << "Gas"
    heatingcoilType = OpenStudio::Ruleset::OSArgument::makeChoiceArgument("heatingcoilType", heatingcoilChs,true)
    heatingcoilType.setDisplayName("Heating Coil Type")
    heatingcoilType.setDefaultValue("Gas")
    args << heatingcoilType

	#make an argument for economizer control 
    economizer_option = OpenStudio::Ruleset::OSArgument::makeBoolArgument("economizer_option",true)
    economizer_option.setDisplayName("Economizer Control?")
    economizer_option.setDefaultValue(true)
    args << economizer_option
	
	#make an argument for ERV 
    erv_option = OpenStudio::Ruleset::OSArgument::makeBoolArgument("erv_option",true)
    erv_option.setDisplayName("Energy recovery ventilation device?")
    erv_option.setDefaultValue(true)
    args << erv_option
		

    #make an argument to apply recommended availability ventilation schedules 
  ...
  def run(model, runner, user_arguments)
	...
	controlZoneSpaceType = runner.getOptionalWorkspaceObjectChoiceValue("controlZoneSpaceType", user_arguments, model)
	coolingcoilCOP = runner.getDoubleArgumentValue("coolingcoilCOP",user_arguments)
	heatingcoilCOP = runner.getDoubleArgumentValue("heatingcoilCOP",user_arguments)

    remake_schedules = runner.getBoolArgumentValue("remake_schedules",user_arguments)
	economizer_option = runner.getBoolArgumentValue("economizer_option",user_arguments)
	erv_option = runner.getBoolArgumentValue("erv_option", user_arguments)
	heatingcoilType= runner.getStringArgumentValue("heatingcoilType", user_arguments)
	...
	controlZoneSpaceTypeCheck = OsLib_HelperMethods.checkOptionalChoiceArgFromModelObjects(controlZoneSpaceType, "controlZoneSpaceType", "to_SpaceType", runner, user_arguments)
	if controlZoneSpaceTypeCheck==false then return false else controlZoneSpaceType=controlZoneSpaceTypeCheck["modelObject"] end 
    ...
    primaryHVAC = {"doas" => false, "multistage"=> true, "fan" => "OnOff", "heat" => heatingcoilType, "cool" => "MultiDX", "economizer" => economizer_option, "erv" => erv_option, "coolcoilCOP"=>coolingcoilCOP, "heatcoilCOP"=>heatingcoilCOP, "controlZoneSpaceType"=>controlZoneSpaceType}
    ...
    options["zoneHVAC"] = zoneHVAC
	options["coolingcoilCOP"] = coolingcoilCOP
    if primaryHVAC["doas"]
...
SingleZoneVAVRTUWithTwoSpeedFanControl.new.registerWithApplication